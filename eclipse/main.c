...
//	//Temperaturas
int16_t Temper_Precalentamiento = 26;
    		...
    		if (++c == 2)//20ms
			{
                ...
                if (++MAX6675_ConversionTime_access == 11)//MAX6675 has max 0.22s
                {
                    ...
                    else
                    {
						...
						{
							temper_measured = ( (int16_t) (temper_mean_val>> TEMPER_K_MULT_DIV_SHIFT) ) + TmprFactCorr; //div usando shift
							//
							temper_mean_n_samples = 0x00;
							temper_mean_val = 0x00;
    	...
    	if (pinGetLevel_level(PGLEVEL_LYOUT_SWONOFF) == 0)
    	{
    			...
    			else if (sm0 == 1)
    			{
    				if (igDeteccFlama_doJob())
    				{
    					...
    					lcdan_print_PSTRstring(PSTR("Detec.FlameOK"));
    				}
    			}
    			else if (sm0 == 2)
    			{
    				...
    				if (temper_measured >= Temper_Precalentamiento)
    				{
    					...
    					lcdan_set_cursor(0, 1);
    					lcdan_print_PSTRstring(PSTR("Precalenta.OK"));
    			...
    			else if (sm0 == 3)
    			{
    				//StartStop left ?
    				//StartStop right ?
    				//timer_left;//descuenta c/independientemente
    				//timer_right;
    			}
    	}
    	else//OFF
